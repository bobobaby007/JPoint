...
class UserImageList:UIViewController,UITableViewDataSource,UITableViewDelegate,UIAlertViewDelegate{
    ...
    func _ifHasImages(){//--------没发布过图片
        if self._imagesArray.count == 0{
            if _btn_noImage == nil{
                ...
                _btn_noImage?.setTitle("未发布过图片", forState: UIControlState.Normal)
