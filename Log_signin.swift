...
//        _white.backgroundColor = UIColor(white: 1, alpha: 0.1)
        ...
        _txt_nickname = UITextField(frame: CGRect(x: _gap, y: _barH+_gap+2*_buttonH, width: self.view.frame.width-2*_gap, height: _buttonH))
        _txt_nickname?.textColor = UIColor.whiteColor()
        //_txt_nickname?.font = _font_cell_title_normal
        ...
        _txt_password = UITextField(frame: CGRect(x: _gap, y: _barH+_gap+3*_buttonH, width: self.view.frame.width-2*_gap, height: _buttonH))
...
//        _btn_go?.titleLabel?.font = _font_cell_title
    ...
    func timerHander(timer:NSTimer){
       // print(timer)
        ...
        _btn_getSmscode?.setTitleColor(UIColor.lightGrayColor(), forState: UIControlState.Normal)
        _btn_getSmscode?.layer.borderColor = UIColor.lightGrayColor().CGColor
...
//            _btn_getSmscode?.titleLabel?.font = _font_cell_title_normal
    ...
    //----注册
    func _go(){
        MainAction._signup(_txt_mobile!.text!, __pass: _txt_password!.text!,__nickname: _txt_nickname!.text!) { (__dict) -> Void in
    ...
    //----发送验证码
    func _sendSmscode(){
//        _btn_getSmscode?.setTitleColor(_color_gray_description, forState: UIControlState.Normal)
